...
log_dir = "/home/tanujthakkar/ash/PRG-Hoop-Recognition/training/logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
writer = SummaryWriter(log_dir=log_dir)
...
# Create a training set that uses specified quantity of the images in dataset and a validation set
# that uses remaining images in the dataset
dataset = hoop_dataset.HoopDataset("../assets/hoops", "../assets/training_data", "../assets/masks")
prelabeled_dataset = hoop_dataset.HoopDataset("../assets/masks", "../assets/backgrounds_with_hoops", "../assets/masks",preset=True)
dataset_size = len(dataset)
TRAINSET_SIZE = int(0.8*dataset_size)
VALIDATIONSET_SIZE = dataset_size - TRAINSET_SIZE
...
#Middle image: Model prediction of where the hoop is
#Right image: Actual location of where the hoop is
def visualize_sample(image, pred_mask, mask):
    image_numpy = image.detach().cpu().numpy().transpose(1, 2, 0)
    pred_mask_numpy = pred_mask.detach().cpu().numpy().transpose(1, 2, 0)
    mask_numpy = mask.detach().cpu().numpy().transpose(1, 2, 0)
...
dataloader_length = len(dataloader)
training_run = len(os.listdir('../output'))
print(os.listdir('../output'))
os.mkdir('../output/training'+str(training_run))
for epoch in range(NUM_EPOCHS):
    #Training loop
    ...
    for i, (real_i, image, mask) in enumerate(dataloader):    
        try:
            ...
            #Crop image appropriately
            # start_time = time.perf_counter()
            # crop_transform = lambda im : torchvision.transforms.functional.crop(im, top=20, left=20, height=352, width=592)
            ...
            print((epoch, dataloader_length, i,))
            if(i % 1 == 0):
                result = visualize_sample(image[0], pred_mask[0], mask[0])
                result = tf.transpose(result, perm=[1,2,0])
                result = result.numpy()
                result = (result * 255).astype(np.uint8)
                plt.imsave('../output/training'+str(training_run)+'/epoch'+str(epoch)+'sample'+str(i)+'.png', result)
                #writer.add_image('Training ' + str(epoch) + ' ' + str(i), visualize_sample(image[0], pred_mask[0], mask[0], real_i[0]), dataformats='CHW')
    ...
    for i, (real_i, image, mask) in enumerate(validateloader):
        try:
            ...
            #Add binary cross entropy loss of item to total loss of all items
            loss = F.binary_cross_entropy(pred_mask, mask, weight=weight).detach()
            total_loss += loss.cpu()
